...
{l.has(c)||(c.addEventListener("ended",()=>{h(b,"Video Ended",c)}),c.addEventListener("play",()=>{h(b,"Video Started",c)}),c.addEventListener("pause",()=>{h(b,"Video Paused",c)}),l.add(c))})}function k(b,c){const e=`data-video-${c.toLowerCase()}`;b=b.getAttribute(e);return null!==b?"true"===b:g[c]}function u(b){b.getElementById(b.currentSceneId()).querySelectorAll("video").forEach(c=>{k(c,"autoMute")&&(c.muted=!0);k(c,"autoInline")&&(c.playsInline=!0);requestAnimationFrame(()=>{k(c,"autoStart")&&
...
callback:function(b,c,e){t(b);u(b)}});window.HYPE_eventListeners.push({type:"HypeSceneUnload",callback:function(b,c,e){v(b)}});return{version:"1.0.5",setDefault:function(b,c){"object"===typeof b?Object.assign(g,b):g[b]=c},getDefault:function(b){return b?g[b]:{...g}}}}());
